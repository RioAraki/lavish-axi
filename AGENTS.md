...
1. `lavish-axi <file.html>` (`openCommand`) -> POST `/api/sessions` -> `SessionStore.upsertSession` -> the CLI opens the returned session URL in the browser, then makes exactly one bounded `GET /api/:key/layout-audit` call and returns.
   ...
   A session record carries identity and timestamps (`key`, `file`, `slug`, `url`, `opened_at`, `updated_at`) plus the one human-set field, `favorite`. There is no status, no ended-by attribution, and no reopen etiquette to encode, because a session is nothing more than "this file is being displayed at this URL"; fields written by older releases are ignored and dropped on the next upsert. `favorite` is the deliberate exception to that drop - it is written only when true, and `upsertSession` carries it across re-opens, because re-displaying a page must not unprotect it from the index's 7-day prune (README's Favorites bullet owns the user-facing rule).
